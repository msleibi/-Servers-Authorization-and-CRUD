...
def newRestaurant():
    if request.method == 'POST':
        ...
        session.add(newRes)
        session.commit()
        flash("new restaurant " + newRes.name  + " created!")
...
def editRestaurant(restaurant_id):
    ...
    if request.method =='POST':
        ...
        session.add(editRes)
        session.commit()
        flash("Restaurt has been edited")
        return redirect(url_for('restaurantAll'))
    else:
...
def deleteRestaurant(restaurant_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash(deleteRes.name + " has been deleted")
        return redirect(url_for('restaurantAll'))
    else:
...
def restaurantMenu(restaurant_id):
    ...
    restaurantOne = session.query(Restaurant).filter_by(id=restaurant_id).one()
    items = session.query(MenuItem).filter_by(restaurant_id=restaurant_id)
    if 'username' not in login_session:
	    return render_template('publicmenu.html',restaurant=restaurantOne, items=items, restaurant_id=restaurant_id)
    return render_template(
        'menu.html', restaurant=restaurantOne, items=items, restaurant_id=restaurant_id)
...
def deleteMenuItem(restaurant_id, menu_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deletemenuitem.html', restaurant_id=restaurant_id,menu_id=menu_id,items=itemToDelete)
